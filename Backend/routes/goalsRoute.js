...
router.post('/goals/:goalId/request', ensureAuthenticated, async (req, res) => {
  try {
    ...
    // Get the user ID, handling both _id and id properties
    const userId = req.session.user._id || req.session.user.id;
    if (!userId) {
      return res.status(401).json({ success: false, error: 'User not authenticated' });
    }
    
    // Check if user is the child who owns the goal
    ...
    // Update goal with request status
...
router.post('/goals/:goalId/request-response', ensureAuthenticated, async (req, res) => {
  try {
    ...
    // Check if user is a parent of the goal
    ...
    if (response === 'denied') {
      // Add the collected amount back to child's balance
      ...
      // Just update the request status for approval
